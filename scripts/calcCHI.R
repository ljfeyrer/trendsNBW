...
# - Threat intensity is normalized as decile % classes of the underlying data values
# or as presence absence of stressor footprint so all layers range from from 0-1 
# - As all threats considered are assessed as serious for NBW, all stressors are considered on the same scale
...
# intensity of differences in specific serious threats to NBW over time
...
# as stars
Impactpre_df = st_as_stars(pre_CHI)
...
#Pre individual threat plots---------
#set up facet scale
# scale_params <- tibble::tibble(
#   facet_var =  "Oil & Gas Operations",
#   location = "br",
#   width_hint = 0.25
# )
...
plotThreats1
 ...
 #post plot map -----
 # as stars
 Impactpost_df = st_as_stars(post_CHI)
